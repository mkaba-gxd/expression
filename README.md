...
指定された sample ID または flowcell ID から検体情報をデータベースを検索し、CAPサーバ/バックアップサーバ内のファイルを抽出してファイルを作成します。そのため、データベースに登録がない検体や、規程の場所に解析データ（途中ファイル）が格納されていない検体に対しては実行できません。\
**このツールは正式な検証を経ていません。** 不具合等が生じた場合は適宜修正するか、手動で必要なデータを抽出・加工してください。\
**データベースの設計内容が不明なため、データベース検索時に想定外の動作を行う可能性があります。**

| command        | 概要                                          |
|:---------------|:----------------------------------------------|
|[batch, BC](#BC)        |指定のバッチで解析された発現量値を検体別に出力する |
